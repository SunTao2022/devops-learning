...
## 八、证书备考计划（2026-08-21 定）

> **路线共识**：CCNP + AZ-104（已有）+ CKA + Terraform Associate（大项目前考）+ AZ-400（工作后远期）

### 近期备考（大项目之前）

| 证书 | 考试形式 | 准备时间 | 费用 | 状态 |
|------|---------|---------|------|------|
| **CKA**（K8s 管理员） | 实操（真实集群排障） | 2-3 周刷题 | ~$395 | 📌 计划 |
| **Terraform Associate** | 选择题 | 1-2 周 | ~$70 | 📌 计划 |

**顺序逻辑**：先考证书（证明能力）→ 再做综合项目（展示能力）→ 项目质量因能力提升而更好。

### 远期目标（工作 1-2 年后）

| 证书 | 路线 | 说明 |
|------|------|------|
| AZ-400（Azure DevOps Expert） | DevOps 路线 | 与 DevOps/SRE 100% 重合，比 AZ-305 贴合 |
| AZ-305（Azure Architect Expert） | 架构师路线 | 二选一，按职业方向定 |

### 明确不考（避坑）

- AWS 认证（深耕 Azure 一个方向，不分散）
- CKAD（偏开发，CKA 才是运维方向）
- CKS（太进阶，工作后再说）
- RHCSA（CKA 已隐含 Linux 基础）
- Prometheus 认证（小众，招聘不要求）
